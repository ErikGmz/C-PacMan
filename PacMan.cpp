...
Mapas llenar_mapa1();
Mapas llenar_mapa2();
void movimiento_pacman(char*[], char*[]);
void continuar();
void checar_records();
...
void comenzar_juego(ALLEGRO_DISPLAY* pantalla) {
    ...
    for (int i = 0; i < 101; i++) {
        *(mapa_01 + i) = primero.mapa1[i];
        *(mapa_02 + i) = segundo.mapa2[i];
    }

    movimiento_pacman(mapa_01, mapa_02);
}
    ...
    while (!finalizado) {
        ...
        case ALLEGRO_EVENT_KEY_DOWN:
            if(evento.keyboard.keycode == ALLEGRO_KEY_ENTER) {
                ...
                if(auxiliar != '+' && strlen(cadena) < 8) al_play_sample(tecla, 0.6, 0, 1.0, ALLEGRO_PLAYMODE_ONCE, 0);
                fflush(stdin);
...
Mapas llenar_mapa1() {
    ...
    for (int i = 0; i < 101; i++) {
        strcpy_s(*(tablero.mapa1 + i), 91, mapa[i]);
    }
    return tablero;
}
...
Mapas llenar_mapa2() {
    ...
    for (int i = 0; i < 101; i++) {
        strcpy_s(*(tablero.mapa2 + i), 91, mapa[i]);
    }
    return tablero;
}


void movimiento_pacman(char* mapa_01[], char* mapa_02[]) {
    enum { abajo = 0, arriba = 4, izquierda = 8, derecha = 12 };
    char nombre_archivo[30], numero[3];
    int coord_x = 285, coord_y = 345, velocidad = 5, animacion = 0, direccion = abajo, direccion_previa = abajo;
    ...
    while (!terminado) {
        ...
        if(evento.type == ALLEGRO_EVENT_TIMER) {
            ...
            if (al_key_down(&estado_tecla, ALLEGRO_KEY_DOWN)) {
                if (mapa_02[((coord_y - 70) / 5) + 2][(coord_x - 66) / 5] != '*' && ((coord_y - 70) / 5) + 1 <= 100) direccion = abajo;
            }
            if (al_key_down(&estado_tecla, ALLEGRO_KEY_UP)) {
                if (mapa_02[((coord_y - 70) / 5) - 2][(coord_x - 66) / 5] != '*' && ((coord_y - 70) / 5) - 1 >= 0) direccion = arriba;
            }
            if (al_key_down(&estado_tecla, ALLEGRO_KEY_LEFT)) {
                if (mapa_02[(coord_y - 70) / 5][((coord_x - 66) / 5) - 2] != '*' && ((coord_x - 66) / 5) - 1 >= 0) direccion = izquierda;
            }
            if (al_key_down(&estado_tecla, ALLEGRO_KEY_RIGHT)) {
                if (mapa_02[(coord_y - 70) / 5][((coord_x - 66) / 5) + 2] != '*' && ((coord_x - 66) / 5) + 1 <= 87) direccion = derecha;
            }

            switch (direccion) {
            case abajo:
                if (mapa_02[((coord_y - 70) / 5) + 1][(coord_x - 66) / 5] != '*' && ((coord_y - 70) / 5) + 1 <= 100) {
                    coord_y += velocidad;
                    animacion++;
                }
                else {
                ...
                break;
            case arriba:
                if (mapa_02[((coord_y - 70) / 5) - 1][(coord_x - 66) / 5] != '*' && ((coord_y - 70) / 5) - 1 >= 0) {
                    coord_y -= velocidad;
                    animacion++;
                }
                else {
                ...
                break;
            case izquierda:
                if (mapa_02[(coord_y - 70) / 5][((coord_x - 66) / 5) - 1] != '*' && ((coord_x - 66) / 5) - 1 >= 0) {
                    coord_x -= velocidad; 
                    animacion++;
                }
                else {
                ...
                break;
            case derecha:
                if (mapa_02[(coord_y - 70) / 5][((coord_x - 66) / 5) + 1] != '*' && ((coord_x - 66) / 5) + 1 <= 87) {
                    coord_x += velocidad;
                        animacion++;
                }
                else{
        ...
        al_draw_bitmap(mapa2, 76, 80, NULL);
        al_flip_display();
        al_clear_to_color(al_map_rgb(0, 0, 0));
    }
}
